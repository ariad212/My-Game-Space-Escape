...
function draw() {
  ...
  if(gameState === PLAY) {
    textSize(25);
    text("Use the arrow keys to succesfully score 50 points by collecting the coins and dodging the obstacles that come your way. Good luck!", width - 1800, 100);
    ...
    if(lives <= 0) {
      textSize(40);
      text("You lost! Better luck next time...", width/2 - 222, height/2 - 150);
    }

    if(score >= 50) {
      textSize(40);
      text("You won!! Thanks for playing!", width/2 - 222, height/2 - 150);
    }
  ...
  text("Lives: " + lives, width - 200, 140);
...
function spawnPlanets() {
  ...
  planet = createSprite(width + 50, random(height/2 - 285, height/2 + 400), 50, 50);
  planet.velocityX -= 3;
  planet.scale = 0.3;
...
function spawnObstacles() {
  if(frameCount % 60 == 0) {
    ...
    obstacle = createSprite(width + 50, random(height/2 - 285, height/2 + 400), 50, 50);
